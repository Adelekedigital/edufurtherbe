...
class TestStateFor:
    ...
    def test_the_bands(self, balance: int, expected: CreditState) -> None:
        ...
        assert state_for(balance, LADDER) == expected
    ...
    def test_a_balance_above_five_is_still_on_track(self) -> None:
        ...
        one."""
        assert state_for(6, LADDER) == CreditState.ON_TRACK
...
class TestAllowanceFor:
    def test_the_ordinary_case_is_the_steady_state_ceiling(self) -> None:
        for balance in (0, 1, 2, 3, 4):
            assert allowance_for(balance, LADDER) == LADDER.steady_state

    def test_the_bar_moves_when_a_credit_is_spent(self) -> None:
        """**The defect this replaced, stated as a test.**

        Dividing by `LADDER.monthly` made `balance == allowance` for every
        ...
        """
        spending = [allowance_for(b, LADDER) for b in (4, 3, 2, 1, 0)]

        assert spending == [LADDER.steady_state] * 5
    ...
    def test_a_balance_above_the_ceiling_raises_it(self) -> None:
        ...
        assert allowance_for(5, LADDER) == 5
        assert allowance_for(7, LADDER) == 7

    def test_the_boundary_is_not_off_by_one(self) -> None:
        assert allowance_for(LADDER.steady_state, LADDER) == LADDER.steady_state

    def test_the_ceiling_is_the_starter_plus_the_month(self) -> None:
        assert LADDER.steady_state == LADDER.starter + LADDER.monthly == 4

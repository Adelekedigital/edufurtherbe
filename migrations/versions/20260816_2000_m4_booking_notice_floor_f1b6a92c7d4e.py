...
# `d1f8a3c62b47` — step two of the enum conversion — and steps three and four
# merged first, so leaving it there would have given the chain **two heads** and
# made `alembic upgrade head` refuse to run. This touches `min_notice_minutes`,
# which none of those steps go near, so the order between them carries no
# meaning beyond which landed first.
down_revision = "f5c3a81e6b29"
branch_labels = None
depends_on = None

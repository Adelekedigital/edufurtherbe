...
class User(TimestampMixin, Base):
    ...
    # NO `default` AND NO `server_default`. This is the one table whose id is not
    # generated by the database, and ADR 0009 §9 names the assertion: `users.id`
    # *is* the Supabase auth user id. A default would quietly mint a
    # valid-looking id for any insert that forgot to pass the auth one, producing
    # a row that can never be logged into and looks entirely normal.
    #
    # It is therefore also the one table whose id is not time-ordered — Supabase
    # issues v4. Nothing downstream depends on that ordering, and `created_at`
    # carries Bubble's signup date on migrated rows, so a UUIDv7 was never usable
    # as a sort key here anyway.
...
class UserProfile(TimestampMixin, Base):
    ...
    **The dev extract shows the reverse** — the coded field is empty on all 19
    ...
    current_country_code: Mapped[str | None] = mapped_column(
        CHAR(2), ForeignKey("countries.code", ondelete="RESTRICT")
    )
    ...
    __table_args__ = (
        Index("ix_user_profiles_origin_country", "origin_country_code"),
        Index("ix_user_profiles_current_country", "current_country_code"),
    )
...
class UserLanguage(TimestampMixin, Base):
    ...
    **The M1c transform will violate this foreign key**, and it is worth knowing
    ...
    language_code: Mapped[str] = mapped_column(
        CHAR(3),
        ForeignKey("languages.code_639_3", ondelete="RESTRICT"),
        primary_key=True,
    )
    ...
    __table_args__ = (
        Index("ix_user_languages_language", "language_code"),
